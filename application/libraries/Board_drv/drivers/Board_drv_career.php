 ...
 * @history
 * */
class CI_Board_drv_career extends CI_Driver 
{
	...
	public function get_list($parm)
	{
		...
		if( isset($parm['search_val']) === TRUE && empty($parm['search_val']) === FALSE )
		{
			switch($parm['search_key'])
			{
				...
				case 'all':
					$this->db->like('a.title', $parm['search_val']);
					$this->db->or_like('a.contents', $parm['search_val']);
				break;
			}
		}
	...
	public function get_top_data($count = 3)
	{
		...
		if( $this->_debug == 'Y' )
		{
			$this->add_error($this->board_table_name.'_gettopdata_query', $this->db->last_query, 'debug');
		}
