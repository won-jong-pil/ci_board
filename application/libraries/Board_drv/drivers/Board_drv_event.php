...
/**
 * @title 게시판 - 이벤트 게시판 클래스
 * @author - 원종필(won0334@chol.com)
 * @history
 * */
class CI_Board_drv_event extends CI_Driver 
{
	...
	public function insert($config)
	{
    	...
    	$data['reply_use'] = $this->input->post('reply_use');
    	$data['status'] = $this->input->post('status');
    	$data['anno_date'] = $this->input->post('anno_date');
    	$data['event_type'] = strtoupper($this->input->post('event_type'));
    	$data['apply_use'] = strtoupper($this->input->post('apply_use'));
    	$data['mobile_contents'] = $this->input->post('mobile_contents');
    	$data['use_type'] = $this->input->post('use_type');
    	$data['event_division'] = $this->input->post('event_division');
	...
	public function delete($idx, $config)
	{
		...
		{		
			...
			$this->db->where('brd_code', $this->board_code);
			$this->db->where('brd_idx', $old_data['result']['idx']);	
			$result[$idx_key.'_winer_db_result'] = $this->db->delete('board_winner');
			$this->add_error($idx_key.'_winer_db_result', $result[$idx_key.'_winer_db_result'], 'debug');
		}
	...
	public function get_list($parm)
	{
		...
		$this->db->select('SQL_CALC_FOUND_ROWS a.*, board_winner.idx winner_idx, board_winner.contents winner_contents', FALSE);
		$this->db->join('board_winner', 'board_winner.brd_code = \''.$this->board_code.'\' and board_winner.brd_idx = a.idx', 'left');
		...
		if( isset($parm['user_idx']) === TRUE && empty($parm['user_idx']) === FALSE )
		{
			$this->db->select('event_app.user_idx app_idx');
			$this->db->join('event_app', 'event_app.event_idx = a.idx', 'left');
			$this->db->where('event_app.user_idx', $parm['user_idx']);
		}
		if( isset($parm['winner_flag']) === TRUE && empty($parm['winner_flag']) === FALSE ) $this->db->where('board_winner.idx is not ', 'null', FALSE);
		if( isset($parm['winner_view_flag']) === TRUE && empty($parm['winner_view_flag']) === FALSE ) $this->db->where('board_winner.status', $parm['winner_view_flag']);
		if( isset($parm['view_flag']) === TRUE && empty($parm['view_flag']) === FALSE ) $this->db->where('view_flag', $parm['view_flag']);
		if( isset($parm['type']) === TRUE && empty($parm['type']) === FALSE ) $this->db->where('event_type', $parm['type']);
		...
		if( isset($parm['use_type']) === TRUE && empty($parm['use_type']) === FALSE ) $this->db->where_in('a.use_type', array($parm['use_type'], 'A'));

		$in_date = 'start_date <= \''.date('Y-m-d').'\' and end_date >= \''.date('Y-m-d').'\'';
		$out_date = 'end_date < \''.date('Y-m-d').'\'';
		$this->CI->db->from('(select *, (case
