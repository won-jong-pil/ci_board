...
/**
 * @title 게시판 - 이벤트 참여자 게시판 클래스
 * @author - 원종필(won0334@chol.com)
 * @history
 * */
class CI_Board_drv_event_app extends CI_Driver 
{
	...
	public function get_list($parm)
	{
		...
		$this->db->select('SQL_CALC_FOUND_ROWS a.*, board_event.title', FALSE);
		$this->db->from($this->board_table_name.' a');
		$this->db->join('board_event', 'a.event_idx = board_event.idx');
		if($this->get_attr('etc.board_file_use') == 'Y')
		{
		...
		if( isset($parm['search_key']) === TRUE && empty($parm['search_key']) === FALSE && isset($parm['search_val']) === TRUE && empty($parm['search_val']) === FALSE )
		{
			...
			{
				case 'title':
					$this->db->like('board_event.title', $parm['search_val']);
				break;
				
				case 'name':
					$this->db->like('users.name', $parm['search_val']);
				break;
		...
		if( isset($parm['user_idx']) === TRUE && empty($parm['user_idx']) === FALSE ) $this->db->where('a.user_idx', $parm['user_idx']);
	...
	{
		$this->db->select('a.*, board_event.title, board_event.event_division');
