 ...
 * @history
 * */
class CI_Board_drv_movie extends CI_Driver 
{
	...
	{
		$product_movie = $this->input->post('product_movie');
		$tag = $this->input->post('tag');
    	$data['title'] = $this->input->post('title');
    	$data['cate'] = $this->input->post('cate');
    	$data['contents'] = $this->input->post('contents');
    	$data['doctor_info_use'] = $this->input->post('doctor_info_use');
    	$data['doctor_info_name'] = $this->input->post('doctor_info_name');
    	$data['doctor_info_hospital'] = $this->input->post('doctor_info_hospital');
    	$data['doctor_info_sector'] = $this->input->post('doctor_info_sector');
    	$data['reg_date_use'] = $this->input->post('reg_date_use');
    	$data['reg_date'] = $this->input->post('reg_date');
    	$data['shooting_date_use'] = $this->input->post('shooting_date_use');
    	$data['shooting_date'] = $this->input->post('shooting_date');
    	$data['use_prd_use'] = $this->input->post('use_prd_use');
    	$data['movie_use'] = $this->input->post('movie_use');
    	$data['main_use'] = $this->input->post('main_use');
    	$data['hot_movie'] = $this->input->post('hot_movie');
    	$data['new_movie'] = $this->input->post('new_movie');
    	$data['vimeo_idx'] = $this->input->post('vimeo_idx');
    	
    	if(empty($tag) === FALSE && $tag != FALSE) $data['tag'] = json_encode($tag);
    	if(empty($data['movie_use']) === TRUE) $data['movie_use'] = 'N';
    	if(empty($data['hot_movie']) === TRUE) $data['hot_movie'] = 'N';
    	if(empty($data['new_movie']) === TRUE) $data['new_movie'] = 'N';

		$result['result'] = $this->db->insert($this->board_table_name, $data);
		$insert_idx = $this->db->insert_id();
		
		if(is_array($product_movie) === TRUE && count($product_movie) > 0)
		{
			foreach($product_movie as $key=>$list)
			{
				$product_data = array(
					'movie_idx'=>$insert_idx,
					'product_movie_idx'=>$list
				);
				$result['result'] = $this->db->insert('product_movie_relation', $product_data);
			}
		}
	...
	public function update($idx, $config)
	{
		...
		if(is_array($product_movie) === TRUE && count($product_movie) > 0)
		{
			$this->db->where('movie_idx', $idx);
			$this->db->delete('product_movie_relation');
	...
	public function get_list($parm)
	{
		...
		$this->db->select('SQL_CALC_FOUND_ROWS a.*, movie_info.vimeo_link, movie_info.vimeo_id', FALSE);
		$this->db->from($this->board_table_name.' a');
		$this->db->join('movie_info', 'a.vimeo_idx = movie_info.idx', 'left');
		...
		if( isset($parm['movie_use']) === TRUE || empty($parm['movie_use']) === FALSE) $this->db->where('movie_use', $parm['movie_use']);
		if(isset($parm['main_use']) === TRUE && empty($parm['main_use']) === FALSE) $this->db->where('main_use', $parm['main_use']);
		if( isset($parm['per_page']) === TRUE && empty($parm['per_page']) === FALSE && isset($parm['off_set']) === TRUE  ) $this->db->limit($parm['per_page'], $parm['off_set']);
		if( isset($parm['cate']) === TRUE && empty($parm['cate']) === FALSE && isset($parm['sub_cate']) === FALSE ) $this->db->where('cate', $parm['cate']);
		if( isset($parm['sub_cate']) === TRUE && empty($parm['sub_cate']) === FALSE ) $this->db->where_in('cate', $parm['sub_cate']);
		if( isset($parm['tag']) === TRUE && empty($parm['tag']) === FALSE ) $this->db->like('tag', $parm['tag']);
		...
		if
		( 
			(isset($parm['sub_cate']) === TRUE && empty($parm['sub_cate']) === FALSE) OR 
			(isset($parm['search_val']) === TRUE && empty($parm['search_val']) === FALSE) OR 
			(isset($parm['tag']) === TRUE && empty($parm['tag']) === FALSE) 
		)
		{
			foreach($result['result'] as $key=>$list)
			{
				$result['result']['cate'][$list['cate']][] = $list;
			}
		}
		...
		if( $this->_debug == 'Y' )
		{
			$this->add_error($this->board_table_name.'_select_parm', @json_encode($parm), 'debug');
		}
	...
	{
		$this->db->select('a.*, movie_info.vimeo_link, movie_info.vimeo_id');
		...
		$this->db->from('product_movie_relation a');
		$this->db->join('board_product', 'a.product_movie_idx = board_product.idx', 'left');
		$this->db->where('a.movie_idx', $idx);
		$this->db->where('board_product.movie_use', 'Y');
		$result['product_info'] = $this->db->get()->result_array();
