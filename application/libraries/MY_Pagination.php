...
class MY_Pagination extends CI_Pagination {
	public $block = 0;//현재 보여지는 페이징 블럭
	public $total_block = 0;//전체 페이징 블럭
	public $page_size = 10;//한화면에 보여질 페이지리스트 수
	public $display_always = FALSE;//페이지가 1개일때 노출 여부
	public $block_link = TRUE;//다음 페이지 블록으로 가기 노출 여부
	
    public function __construct($config = array())
    {
        parent::__construct($config);
        $this->use_page_numbers = 1;//무조건 페이징 처리시 페이지 번호로만 처리하자.
    }
    	// --------------------------------------------------------------------
	...
	public function create_links()
	{
		...
		if ($num_pages === 1 && $this->display_always === FALSE)
		{
			return '';
		}
		...
		$base_page = 1; //페이징 처리시 게시물수를 기준으로 처리 하지 않고 페이지 번호를 기본으로 사용한다.
		// Are we using query strings?
		if ($this->page_query_string === TRUE)
		{
		...
		$this->total_block = ceil($num_pages / $this->page_size);//전체 페이지 블록수
		$this->block = ceil($this->cur_page / $this->page_size);//현재 페이지 블록		
		$start = ($this->block - 1) * $this->page_size + 1;
		$end = $this->block * $this->page_size;
		if($this->total_block <= $this->block) $end = $num_pages;	
		// And here we go...
		$output = '';

/*		if($this->block_link !== FALSE)
		{
			if($this->block > 1)
			{
